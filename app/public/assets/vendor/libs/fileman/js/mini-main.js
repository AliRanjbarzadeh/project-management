...
RoxyUtils.FormatDate = function (date, format) {
	var ret = '';
	try {
		ret = $.format.date(date, format ?? RoxyFilemanConf.DATETIMEFORMAT);
	} catch (ex) {
		//alert(ex);
		ret = date.toString();
...
function Directory(fullPath, numDirs, numFiles) {
	...
	this.GetHtml = function () {
		...
		html += '<div><img src="' + assetsUrl + 'images/' + (this.dirs > 0 ? 'dir-plus.png' : 'blank.gif') + '" class="dirPlus" width="9" height="9">';
		html += '<img src="' + assetsUrl + 'images/folder.png" class="dir"><span class="name">' + this.name + ' (' + this.files + ')</span></div>';
		html += '</li>';
	...
	this.Select = function (selectedFile) {
		...
		el.children('img.dir').prop('src', assetsUrl + 'images/folder.png');
		this.SetStatusBar();
	...
	this.SetOpened = function () {
		...
		if (li.find('li').length < 1)
			li.children('div').children('.dirPlus').prop('src', assetsUrl + 'images/blank.gif');
		else if (this.IsExpanded())
			li.children('div').children('.dirPlus').prop('src', assetsUrl + 'images/dir-minus.png');
		else
			li.children('div').children('.dirPlus').prop('src', assetsUrl + 'images/dir-plus.png');
	};
		...
		$.ajax({
			...
			success: function (dirs) {
				...
				for (i = 0; i < dirs.length; i++) {
					var d = new Directory(dirs[i].p, dirs[i].d, dirs[i].f, dirs[i].time * 1000);
					d.Show();
				}
				$('#pnlLoadingDirs').hide();
				$('#pnlDirList').show();
...
function dragFileOver() {
	$(this).children('img.dir').attr('src', assetsUrl + 'images/folder-green.png');
}

function dragFileOut() {
	$('#pnlDirList').find('img.dir').attr('src', assetsUrl + 'images/folder.png');
}

function makeDragFile(e) {
	var f = new File($(e.target).closest('li').attr('data-path'));
	return '<div class="pnlDragFile" data-path="' + f.fullPath + '"><img src="' + assetsUrl + f.bigIcon + '" align="absmiddle">&nbsp;' + f.name + '</div>';
}
...
function makeDragDir(e) {
	...
	return '<div class="pnlDragDir" data-path="' + f.fullPath + '"><img src="' + assetsUrl + 'images/folder.png" align="absmiddle">&nbsp;' + f.name + '</div>';
}
...
function previewFile() {
	var f = getSelectedFile();
	if (f) {
		window.open(storageUrl + f.fullPath);
	}
}
...
function switchView(t) {
	...
	if (t == 'thumb') {
		$('#pnlFileList .icon').attr('src', assetsUrl + 'images/blank.gif');
